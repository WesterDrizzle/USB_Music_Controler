 ...
 * @return UTF-8 coded character in Little Endian to be compatible with C chars (e.g. '脕', '虐')
 */
static uint32_t lv_txt_unicode_to_utf8(uint32_t letter_uni)
{
...
/**
 * Convert a wide character, e.g. '脕' little endian to be UTF-8 compatible
 ...
 * E.g. in "A脕RT" index of 'R' is 2th char but start at byte 3 because '脕' is 2 bytes long
 ...
 * E.g.: "脕BC" is 3 characters (but 4 bytes)
 ...
 * @param letter_uni an Unicode letter
 * @return ISO8859-1 coded character in Little Endian to be compatible with C chars (e.g. '脕', '虐')
 */
static uint32_t lv_txt_unicode_to_iso8859_1(uint32_t letter_uni)
{
    if(letter_uni < 256)
