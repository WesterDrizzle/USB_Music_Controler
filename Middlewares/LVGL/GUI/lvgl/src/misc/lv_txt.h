 ...
 * @param length length of 'txt' in byte count and not characters (脕 is 1 character but 2 bytes in
 * UTF-8)
 * @param font pointer to a font
 * @param letter_space letter space
 ...
 * @return Encoded character in Little Endian to be compatible with C chars (e.g. '脕', '脺')
 */
extern uint32_t (*_lv_txt_unicode_to_encoded)(uint32_t);

/**
 * Convert a wide character, e.g. '脕' little endian to be compatible with the encoded format.
 ...
 * E.g. in UTF-8 "A脕RT" index of 'R' is 2 but start at byte 3 because '脕' is 2 bytes long
 ...
 * E.g. in UTF-8 "脕BC" is 3 characters (but 4 bytes)
