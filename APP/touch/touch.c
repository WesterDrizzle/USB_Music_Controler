...
u16 TP_Read_AD(u8 CMD)
{
	...
	portENTER_CRITICAL(); // 进入临界区，防止被打断
...
u8 TP_Scan(u8 tp)
{
	...
	if (PEN == 0) // 有按键按下
	{
		...
		{
			tp_dev.x[0] = tp_dev.xfac * tp_dev.x[0] + tp_dev.xoff; // 将结果转换为幕坐标
			tp_dev.y[0] = tp_dev.yfac * tp_dev.y[0] + tp_dev.yoff;
		}
		if ((tp_dev.sta & TP_PRES_DOWN) == 0) // 之前没有被按下
		{
			tp_dev.sta = TP_PRES_DOWN | TP_CATH_PRES; // 键按下
...
void TP_Adj_Info_Show(u16 x0, u16 y0, u16 x1, u16 y1, u16 x2, u16 y2, u16 x3, u16 y3, u16 fac)
{
	...
	LCD_ShowNum(40 + 24, 220, x3, 4, 16);	   // 显���数值
